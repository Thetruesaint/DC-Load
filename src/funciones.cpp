...
void Test_Keypad() {
    ...
    char key = customKeypad.getKey();  // Lee la tecla presionada
    ...
    if (key != NO_KEY && key != lastKey) {
        ...
        tft.fillRect(x - 5, y - 3, 100, 24, TFT_BLACK);

        // Muestra el caracter presionado
        tft.setCursor(x, y);
        tft.printf("Key: %c", key);
    }

    // Si se suelta la tecla, limpiamos el registro
    if (key == NO_KEY) lastKey = 0;
}
...
void Update_TFT(void) {
  ...
  tft.setCursor(8 * cellW, 0 * cellH);
  tft.print(toggle ? F("ON ") : F("OFF"));

  //----------- Valores de corriente, voltaje y potencia ----------- 
  printTFT_Number(0, 1, current, 'A', (current <= 9.999) ? 3 : 2);
  printTFT_Number(7, 1, voltage, 'v',
                  (voltage <= 9.999) ? 3 :
                  (voltage <= 99.99) ? 2 : 1);

  // Potencia
  if (Mode != BC && Mode != CA) {
    tft.setCursor(14 * cellW, 1 * cellH);

    // Borrar campo de potencia
    tft.fillRect(14 * cellW, 1 * cellH, 6 * cellW, cellH, TFT_BLACK);

    if (power < 10) tft.print(power, 2);
    else if (power < 100) tft.print(power, 2);
    else tft.print(power, 1);

    tft.print(F("w"));
  }

  //----------- Encoders (no en modos transitorios)----------- 
  if (Mode != TC && Mode != TL) {
    tft.setCursor(6 * cellW, 2 * cellH);

    // Borrar campo
    tft.fillRect(6 * cellW, 2 * cellH, 6 * cellW, cellH, TFT_BLACK);

    if (Mode == CC || Mode == BC || Mode == CA) {
      tft.print(reading, 3);
    } else {
      if (reading < 100) tft.print("0");
      if (reading < 10)  tft.print("0");
      tft.print(reading, 1);
    }
    ...
    if (blink_cntr == 4) {
      // Borrar un dígito → “parpadeo”
      tft.fillRect(x, y, cellW, cellH, TFT_BLACK);
    }
  }
  ...
  tft.setCursor(0, 4 * cellH + 10);
  tft.printf("encPos: %d", (int32_t)encoderPosition);

  tft.setCursor(0, 4 * cellH + 40);
  tft.printf("getCnt: %d", encoder.getCount());

  tft.setCursor(0, 4 * cellH + 70);
  tft.printf("reading: %.3f", reading);

  tft.setTextColor(TFT_WHITE, TFT_BLACK);
}
//--------------------------- Funciones para el TFT -----------------------------------
void printTFT_Number(int col, int row, float number, char unit, int decimals) {
  int x = col * cellW;
  int y = row * cellH;

  // Borrar campo: 7 caracteres aprox.
  tft.fillRect(x, y, 7 * cellW, cellH, TFT_BLACK);

  tft.setCursor(x, y);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);

  tft.print(number, decimals);

  if (unit == 'A') {
    tft.print("A");   // acá luego podrías meter un símbolo mejor
  } else if (unit != '\0' && unit != ' ') {
    tft.print(unit);
  }
}
